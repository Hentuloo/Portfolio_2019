...
const Markdown = ({ markdown, pageContext: { currentPage }, langContext }) => {
  ...
  const array = markdownAfterRegEX.map((chars, index) => {
    if (`[`.includes(chars)) {
      ...
      return (
        <Dash pageType={pageType} key={index} openDashLiteral={dashLiteral}>
          {chars}
        </Dash>
      );
    }
    ...
    if (chars.includes('"')) {
      return (
        <SentenceInArray pageType={pageType} groupID={groupCount} key={index}>
          {chars}
        </SentenceInArray>
      );
    }
    if (chars.includes(',')) {
      return (
        <Comma pageType={pageType} dashLiteral={dashLiteral} key={index}>
          {chars}
        </Comma>
      );
    }
